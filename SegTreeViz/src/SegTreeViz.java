...
public class SegTreeViz extends Game {
	...
	public SegTreeViz(String title, int width, int height, int FPS) {
		...
		new Thread()
		{
			public void run()
			{
				...
				display.bgcd.addActionListener(new ActionListener(){    

					@Override
					public void actionPerformed(ActionEvent e) {
						display.qr.setText("");
						getArray();
						display.bgcd.setBackground(Color.cyan);
						calBuild=3;
						build(a, 1, 0, n - 1, 1, 1,4);
						display.bgcd.setBackground(Color.white);
					}    
					});
					display.bqr.addActionListener(new ActionListener(){    
						...
						public void actionPerformed(ActionEvent e) {
							display.qr.setText("");
							display.bqr.setBackground(Color.cyan);
							getQuery();
							display.bMin.setBackground(Color.white);
						}    
						}); 
	...
	void build(int a[], int v, int tl, int tr, int x, int y,int id) {
		...
		if (tl == tr) {
			...
			{
                t[v].val= gcd(t[v*2].val , t[v*2+1].val);
			}
			t[v].c1 = t[2*v].mid;
			t[v].c2 = t[2*v + 1].mid;
			t[v].set = true;
	...
	int Querry(int v, int tl, int tr, int qs, int qe,int id) {
        ...
        else if(id==4) // gcd
		{
			...
			cans=gcd(Querry(2*v,tl,tm,qs,qe,id),Querry(2*v+1,tm+1,tr,qs,qe,id));
		}
		t[v].ons=0;
		try {
			Thread.sleep(animSpeed);
